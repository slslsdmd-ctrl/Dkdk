...
# ========== НАПОМИНАНИЕ В ОТДЕЛЬНОМ ПОТОКЕ ==========
        ...
        for uid, user in users.items():
            ...
            if last_reminder:
                ...
                if datetime.now() - last < timedelta(hours=12):
                    continue
            try:
                # Тут нужно будет отправить через бота, но в потоке нельзя
                # Поэтому просто логируем, или используем другой подход
                logger.info(f"Нужно напомнить пользователю {uid}")
                users[uid]['last_reminder'] = datetime.now().isoformat()
                save_data()
            except:
                pass

# ========== TON CONNECT (ЧЕРЕЗ ССЫЛКУ) ==========
# Простой способ: отправляем ссылку на кошелёк
async def connect_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔗 **ПРИВЯЗКА КОШЕЛЬКА** 🔗\n\n"
        "Выберите свой кошелёк для оплаты:\n\n"
        "После установки кошелька, отправьте перевод на адрес ниже:\n\n"
        f"`{TON_WALLET}`\n\n"
        "В комментарии укажите:\n"
        f"`{uid}`\n\n"
        "После перевода нажмите /check_deposit",
...
TEXTS = {
    'ru': {
        ...
        'balance': "💰 БАЛАНС: {} TON",
        'stats': "📊 СТАТИСТИКА\n\nСТАВОК: {}\nПОСТАВЛЕНО: {} TON\nВЫИГРАНО: {} TON\n🔥 ЛУЧШАЯ СЕРИЯ: {}",
        'deposit': "💎 ПОПОЛНЕНИЕ\n\n/deposit [СУММА]\n/check_deposit - проверить платёж",
...
def main_menu(uid):
    keyboard = [
        ...
        [InlineKeyboardButton("🔗 Привязать кошелёк", callback_data="connect_wallet")],
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "change_lang":
        ...
        uid = str(query.from_user.id)
        await query.message.edit_text("💎 ПОПОЛНЕНИЕ", reply_markup=deposit_menu(uid))
...
def main():
    ...
    # Запускаем напоминания в отдельном потоке (чтобы не блокировать event loop)
    reminder_thread = threading.Thread(target=check_reminders_thread, daemon=True)
