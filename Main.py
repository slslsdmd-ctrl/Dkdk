...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ref and ref != uid and ref in users and not users[uid].get("referrer"):
        ...
        users[uid]["balance"] += REFERRAL_REGISTER_BONUS
        save_data()
        await update.message.reply_text(f"🎉 +{REFERRAL_REGISTER_BONUS} TON")
        try:
            await context.bot.send_message(ref, f"🤝 НОВЫЙ РЕФЕРАЛ! {name}")
        except:
            pass
...
async def deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    await update.message.reply_text(get_text(uid, 'deposit', TON_WALLET, MIN_DEPOSIT, uid), reply_markup=main_menu(uid))
